<template>
  <div class="container">
    <form-panel ref="formPanel" :initial-value="formDefaultValue" :list="formList" :formType="type" label-width="90" :scrollContainer="$scrollNode" />
    <virtual-table
      v-if="isShowSubTable"
      ref="table"
      :dataSource="list"
    ...
    >
      <el-button type="primary" icon="el-icon-plus" @click="handleDataCreate">新增</el-button>
      <el-button type="danger" icon="el-icon-delete" @click="handleDataRemove">删除</el-button>
    </virtual-table>
    <div class="info-btn">
...
export default {
  ...
  methods: {
      ...
      return [
        {
          ...
          fieldName: 'dict',
          disabled: false,
          rules: [{ required: true, message: '请输入字典代码', trigger: 'blur' }],
        },
        {
          type: 'INPUT',
          label: '字典名称',
    ...
    async getSubList() {
      this.loading = true;
      const { status, data } = await getSubList({
    ...
    // 提交前统一处理子表变化的数据
    getChildChangeData() {
      const childUpdate = updateData.filter((item) => !deleteData.includes(item.id));
      ...
      const childDel = deleteData.filter((id) => !id.toString().startsWith('new-')).join(',');
      return {
        childUpdate,
        childAdd,
        childDel,
    ...
    // 新建数据
    handleDataCreate() {
      this.$table.INSERT_RECORDS({ id: createUidKey('new-'), sort: 0, stop: '1', dict: this.initialValue.dict }, 'top');
    },
    // 子表数据改变
    dataChangeHandle(data) {
      console.log(data, currentData);
      if (currentData) {
        const type = currentData.id.toString().startsWith('new-') ? createData : updateData;
